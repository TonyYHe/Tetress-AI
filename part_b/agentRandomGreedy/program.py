...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        """
        if self.board._turn_count <= 15:
            return random.choice(self.board.get_legal_actions())
        
        node = Node(self.board, self.color)
        children = node.get_all_children(self.board)
        children = OrderChildren.order_children(self.board, children, self.color, self.transposition_table)
